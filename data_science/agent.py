...
class ToolResult(BaseModel):
    ...
    @classmethod
    def normalize_error(cls, v):
        """Convert error strings to boolean - strings are treated as True (error occurred)."""
        if isinstance(v, bool):
            return v
        elif isinstance(v, str):
            # If error is a string (error message), it means an error occurred
            return True
        elif v is None:
            return False
        else:
...
def ensure_tool_result(payload: Any, tool_name: str = "tool") -> Dict[str, Any]:
    ...
    try:
        if isinstance(payload, dict):
            # [CRITICAL FIX] Normalize __error__ field - convert error strings to boolean
            if "__error__" in payload and isinstance(payload["__error__"], str):
                # If error is a string (error message), convert to boolean
                payload["__error__"] = True
                # Also preserve error message in a separate field
                if "error_message" not in payload:
                    payload["error_message"] = payload["__error__"]
            elif "error" in payload and isinstance(payload["error"], str):
                # Handle 'error' field (non-aliased) - convert string to boolean
                payload["__error__"] = True
                if "error_message" not in payload:
                    payload["error_message"] = payload["error"]
                payload["error"] = True  # Also set as boolean
...
def _intelligent_retry(func, max_retries: int = 3, base_delay: float = 0.5, backoff_factor: float = 2.0, 
...
def _save_tool_markdown_artifact(tool_name: str, display_text: str, tc, result: Optional[Dict] = None) -> str:
    ...
    # CRITICAL: If tc is None, try to recover workspace_root from environment/session state
    # This avoids creating orphaned "default" folders
    if tc is None:
        logger.warning(f"[MARKDOWN ARTIFACT] No tool_context for {tool_name}, attempting to recover workspace...")
        try:
            # Try to get workspace_root from session state or environment
            # This is a last resort - ideally tool_context should always be available
            from .large_data_config import UPLOAD_ROOT
            from pathlib import Path
            import os
            
            # Option 1: Check if there's a recent workspace we can infer
            ...
            if workspaces_root.exists():
                ...
                recent_workspaces = [
                    ...
                    if d.is_dir() and d.name not in ("default", "_global") and (d / "reports").exists()
                ]
                if recent_workspaces:
                    # Use the most recent workspace
                    most_recent = max(recent_workspaces, key=lambda p: (p / "reports").stat().st_mtime if (p / "reports").exists() else 0)
                    # Find the latest run_id subfolder
                    run_folders = [d for d in most_recent.iterdir() if d.is_dir()]
                    if run_folders:
                        latest_run = max(run_folders, key=lambda p: p.stat().st_mtime)
                        reports_dir = latest_run / "reports"
                        reports_dir.mkdir(parents=True, exist_ok=True)
                        md_file = reports_dir / md_name.split("/")[-1]
                        with open(md_file, 'w', encoding='utf-8') as f:
                            f.write(md_body)
                        logger.info(f"[MARKDOWN ARTIFACT] ✅ Saved to inferred workspace: {md_file}")
                        return str(md_file)
            
            # Option 2: If no workspace found, log error but don't create orphaned folders
            logger.error(f"[MARKDOWN ARTIFACT] ❌ Cannot save: No tool_context AND no valid workspace found. Artifact lost.")
            logger.error(f"[MARKDOWN ARTIFACT] This should not happen - ensure tool_context is always provided.")
            return md_name  # Return generic name, but don't create orphaned folders
            
        except Exception as fallback_err:
            logger.error(f"[MARKDOWN ARTIFACT] Fallback recovery failed: {fallback_err}")
    ...
    # Dual-path ensures:
    # 1. Filesystem: Human-readable files in workspace/reports/ (always succeeds)
    # 2. ADK Service: LLM access via LoadArtifactsTool (may fail, retry with backoff)
    
    # STEP 1: Save to filesystem reports/ folder (with retry)
    ...
    def _save_to_filesystem():
        ...
        if tc and hasattr(tc, 'state'):
            ...
            if workspace_root:
                ...
                reports_dir.mkdir(parents=True, exist_ok=True)
                md_filename = md_name.split("/")[-1]  # Extract filename from "reports/xxx.md"
    ...
    filesystem_path = _intelligent_retry(_save_to_filesystem, max_retries=3, base_delay=0.2)
    if filesystem_path:
        filesystem_saved = True
        logger.info(f"[MARKDOWN ARTIFACT] ✅ Filesystem save SUCCESS: {filesystem_path}")
    else:
        logger.error(f"[MARKDOWN ARTIFACT] ❌ Filesystem save FAILED after retries")
    
    # STEP 2: Save to ADK artifact service (with retry, async-aware)
    adk_saved = False
    adk_version = None
    adk_filename = None
    
    if tc and hasattr(tc, 'save_artifact'):
        def _save_to_adk():
            """Inner function for ADK save with retry"""
            try:
                from google.genai import types
                # Create Part with text content (markdown)
                artifact_part = types.Part(text=md_body)
                
                # Determine if context.save_artifact is async
                import inspect
                if inspect.iscoroutinefunction(tc.save_artifact):
                    # Async context - need to await
                    import asyncio
                    try:
                        loop = asyncio.get_event_loop()
                        if loop.is_running():
                            # Loop is running - use thread executor or submit to loop
                            import concurrent.futures
                            future = concurrent.futures.Future()
                            async def _async_save():
                                try:
                                    version = await tc.save_artifact(filename=md_name, artifact=artifact_part)
                                    future.set_result(version)
                                except Exception as e:
                                    future.set_exception(e)
                            asyncio.create_task(_async_save())
                            return future.result(timeout=5)  # Wait max 5 seconds
                        else:
                            return loop.run_until_complete(tc.save_artifact(filename=md_name, artifact=artifact_part))
                    except Exception as async_err:
                        # Fallback: try sync path
                        logger.warning(f"[MARKDOWN ARTIFACT] Async save failed, trying sync: {async_err}")
                        # Try direct call (might work if it's actually sync)
                        return tc.save_artifact(filename=md_name, artifact=artifact_part)
                else:
                    # Sync context
                    return tc.save_artifact(filename=md_name, artifact=artifact_part)
            except ImportError:
                logger.warning(f"[MARKDOWN ARTIFACT] google.genai.types not available")
                raise
            except Exception as e:
                logger.warning(f"[MARKDOWN ARTIFACT] ADK save error: {e}")
                raise
        
        adk_version = _intelligent_retry(_save_to_adk, max_retries=3, base_delay=0.5, backoff_factor=2.0)
        if adk_version is not None:
            adk_saved = True
            adk_filename = md_name
            logger.info(f"[MARKDOWN ARTIFACT] ✅ ADK service save SUCCESS: {md_name} v{adk_version}")
        else:
            logger.warning(f"[MARKDOWN ARTIFACT] ⚠️ ADK service save FAILED after retries (non-critical)")
    
    # STEP 3: Add artifact references to result dict
    if result is not None and isinstance(result, dict):
        if "artifacts" not in result:
            result["artifacts"] = []
        
        # Add filesystem path (always preferred for human access)
        if filesystem_saved and filesystem_path:
            if filesystem_path not in result["artifacts"]:
                result["artifacts"].append(filesystem_path)
            result["artifact_filename"] = filesystem_path
            result["artifact_filesystem_path"] = filesystem_path
        
        # Add ADK reference (for LLM access)
        if adk_saved and adk_filename:
            adk_meta = {"filename": adk_filename, "version": adk_version, "source": "adk"}
            if adk_meta not in result["artifacts"]:
                result["artifacts"].append(adk_meta)
            result["artifact_adk_filename"] = adk_filename
            result["artifact_adk_version"] = adk_version
        
        logger.info(f"[MARKDOWN ARTIFACT] Added artifact references: filesystem={filesystem_saved}, adk={adk_saved}")
    
    # Return priority: filesystem path > ADK filename > generic name
    if filesystem_saved and filesystem_path:
        logger.info(f"[MARKDOWN ARTIFACT] ✅ Returning filesystem path: {filesystem_path}")
        return filesystem_path
    elif adk_saved and adk_filename:
        logger.info(f"[MARKDOWN ARTIFACT] ✅ Returning ADK filename: {adk_filename}")
        return adk_filename
    else:
        logger.warning(f"[MARKDOWN ARTIFACT] ⚠️ Both saves failed, returning generic name: {md_name}")
        return md_name
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        if workspace_root and path_obj.exists():
                                            ...
                                            # Determine workspace subfolder based on file extension
                                            suffix = path_obj.suffix.lower()
                                            if suffix in ('.png', '.jpg', '.jpeg', '.svg', '.gif', '.webp'):
                                                subfolder = "plots"
                                            elif suffix in ('.joblib', '.pkl', '.pickle', '.h5', '.pt', '.pth', '.onnx'):
                ...
                if (part.inline_data and 
                    ...
                    if is_csv:
                        ...
                        try:
                            ...
                            # [CRITICAL FIX] Check workspace/uploads BEFORE calling save_upload()
                            # This prevents creating new files when duplicates already exist
                            import hashlib
                            import base64 as b64_module
                            from pathlib import Path as PathType  # Import Path with alias to avoid conflicts
                            file_hash = None
                            existing_workspace_file = None
                            
                            # Calculate hash of incoming file data
                            try:
                                data_for_hash = file_data
                                if isinstance(data_for_hash, str):
                                    try:
                                        data_for_hash = b64_module.b64decode(data_for_hash, validate=True)
                                    except Exception:
                                        data_for_hash = data_for_hash.encode("utf-8", "ignore")
                                elif not isinstance(data_for_hash, bytes):
                                    data_for_hash = str(data_for_hash).encode("utf-8", "ignore")
                                
                                file_hash = hashlib.sha256(data_for_hash).hexdigest()[:16]
                                logger.info(f"[UPLOAD] Calculated content hash: {file_hash}")
                                
                                # Check workspace/uploads if workspace exists
                                if callback_context and hasattr(callback_context, "state"):
                                    state = callback_context.state
                                    ws_paths = state.get("workspace_paths", {})
                                    ws_uploads = ws_paths.get("uploads")
                                    
                                    if ws_uploads and PathType(ws_uploads).exists():
                                        # Find files with matching content in workspace
                                        for existing_file in PathType(ws_uploads).glob("*.csv"):
                                            try:
                                                if existing_file.stat().st_size == len(data_for_hash):
                                                    with open(existing_file, "rb") as f:
                                                        existing_hash = hashlib.sha256(f.read()).hexdigest()[:16]
                                                    if existing_hash == file_hash:
                                                        existing_workspace_file = existing_file
                                                        logger.info(f"[UPLOAD] ✅ Found identical file in workspace: {existing_file.name}")
                                                        break
                                            except Exception:
                                                continue
                            except Exception as e:
                                logger.warning(f"[UPLOAD] Could not check workspace for duplicates: {e}", exc_info=True)
                            
                            # If identical file exists in workspace, reuse it instead of uploading
                            if existing_workspace_file:
                                logger.info(f"[UPLOAD] Reusing existing workspace file: {existing_workspace_file.name}")
                                filepath_str = str(existing_workspace_file)
                                file_id = existing_workspace_file.name
                                file_size_mb = existing_workspace_file.stat().st_size / 1024 / 1024
                                upload_result = {
                                    "status": "success",
                                    "file_id": file_id,
                                    "bytes": existing_workspace_file.stat().st_size,
                                    "throughput_mb_s": 0,
                                    "message": f"Reused existing file: {file_id}",
                                    "reused": True
                                }
                                logger.info(f"[OK] Reused existing file: {file_id} ({file_size_mb:.2f} MB)")
                            else:
                            ...
                            # [CRITICAL FIX] Derive dataset name FIRST, then create workspace with correct name
                            # This prevents creating workspace with generic "uploaded" name
                            dataset_slug = None
                            if callback_context and hasattr(callback_context, "state"):
                                state = callback_context.state
                                
                                # Try to derive dataset name from original_filename (best source)
                                if original_filename:
                                    import os
                                    import re
                                    clean_name = os.path.splitext(original_filename)[0]
                                    clean_name = re.sub(r'[^a-zA-Z0-9_-]', '_', clean_name)
                                    if clean_name and clean_name.lower() != "uploaded" and not clean_name.lower().startswith("uploaded_"):
                                        state["original_dataset_name"] = clean_name
                                        dataset_slug = clean_name
                                        logger.info(f"[DATASET NAME] Using filename: {dataset_slug}")
                                
                                # If still no name, try to derive from workspace state
                                if not dataset_slug:
                                    try:
                                        # Try reading CSV headers to get a better name
                                        import pandas as pd
                                        if Path(filepath_str).exists():
                                            df = pd.read_csv(filepath_str, nrows=5)
                                            headers = df.columns.tolist()
                                            dataset_slug = derive_dataset_slug(
                                                state,
                                                headers=headers,
                                                filepath=filepath_str,
                                                display_name=original_filename
                                            )
                                            logger.info(f"[DATASET NAME] Derived from CSV: {dataset_slug}")
                                    except Exception as e:
                                        logger.warning(f"Could not derive dataset name from CSV: {e}")
                                        # Final fallback
                                        dataset_slug = "uploaded"
                                
                                # Set dataset name in state for ensure_workspace to use
                                if dataset_slug and dataset_slug != "uploaded":
                                    state["original_dataset_name"] = dataset_slug
                            
                            # [OK] FIX B: Initialize per-dataset workspace with correct dataset name
                            ...
                            if workspace_created and callback_context and hasattr(callback_context, "state"):
                                try:
                                    ...
                                    if ws_uploads:
                                        # [CRITICAL FIX] Check if file with same CONTENT already exists in workspace/uploads
                                        # This prevents creating duplicate files when upload callback runs multiple times
                                        import hashlib
                                        
                                        # Calculate hash of the file being uploaded
                                        file_hash = None
                                        try:
                                            with open(filepath_str, "rb") as f:
                                                file_hash = hashlib.sha256(f.read()).hexdigest()[:16]
                                            logger.info(f"[UPLOAD] File content hash: {file_hash}")
                                        except Exception as e:
                                            logger.warning(f"[UPLOAD] Could not calculate file hash: {e}")
                                        
                                        # Check all CSV files in workspace/uploads for matching content
                                        existing_file_found = None
                                        if file_hash:
                                            ws_uploads_path = Path(ws_uploads)
                                            if ws_uploads_path.exists():
                                                for existing_file in ws_uploads_path.glob("*.csv"):
                                                    try:
                                                        # Compare size first (fast check)
                                                        if existing_file.stat().st_size == Path(filepath_str).stat().st_size:
                                                            # Compare hash
                                                            with open(existing_file, "rb") as f:
                                                                existing_hash = hashlib.sha256(f.read()).hexdigest()[:16]
                                                            if existing_hash == file_hash:
                                                                existing_file_found = existing_file
                                                                logger.info(f"[UPLOAD] Identical file already in workspace: {existing_file.name} (hash: {existing_hash})")
                                                                break
                                                    except Exception as e:
                                                        logger.debug(f"[UPLOAD] Error checking {existing_file}: {e}")
                                                        continue
                                        
                                        if existing_file_found:
                                            # Use existing file - update filepath_str to point to workspace file
                                            filepath_str = str(existing_file_found)
                                            logger.info(f"[UPLOAD] Using existing workspace file: {existing_file_found.name}")
                                            print(f"[OK] File already in workspace: {existing_file_found.name}")
                                        else:
                                            ...
                                            # [CRITICAL FIX] Check for duplicate files with same size, keep oldest
                                            if ws_csv_path.exists():
                                                # File exists - check if it's a duplicate by size
                                                existing_size = ws_csv_path.stat().st_size
                                                new_file_size = Path(filepath_str).stat().st_size if Path(filepath_str).exists() else 0
                                                
                                                if existing_size == new_file_size and new_file_size > 0:
                                                    # Same size - check modification time, keep oldest
                                                    existing_mtime = ws_csv_path.stat().st_mtime
                                                    new_mtime = Path(filepath_str).stat().st_mtime
                                                    
                                                    if new_mtime < existing_mtime:
                                                        # New file is older - replace existing with older version
                                                        logger.info(f"[UPLOAD] Replacing {ws_csv_path.name} with older version")
                                                        ws_csv_path.unlink()  # Delete newer duplicate
                                                        shutil.copy2(filepath_str, str(ws_csv_path))
                                                        filepath_str = str(ws_csv_path)
                                                        logger.info(f"[UPLOAD] ✅ Kept oldest file: {ws_csv_path.name}")
                                                    else:
                                                        # Existing file is older - keep it, delete new upload
                                                        logger.info(f"[UPLOAD] Keeping older file: {ws_csv_path.name} (deleting newer duplicate)")
                                                        if Path(filepath_str).exists() and Path(filepath_str) != ws_csv_path:
                                                            Path(filepath_str).unlink()
                                                            logger.info(f"[UPLOAD] ✅ Deleted newer duplicate: {Path(filepath_str).name}")
                                                        filepath_str = str(ws_csv_path)
                                                else:
                                                    # Different sizes or sizes unknown - skip copy, use existing
                                                ...
                                                # [CRITICAL] After copying, check for ALL duplicate files in workspace/uploads
                                                # If multiple files with same size exist, keep only the oldest
                                                try:
                                                    ws_uploads_path = Path(ws_uploads)
                                                    all_csvs = list(ws_uploads_path.glob("*.csv"))
                                                        ...
                                                        if len(files_list) > 1:
                                                            ...
                                                            for duplicate in files_list[1:]:
                                                                try:
                                                                    duplicate.unlink()
                                                                    deleted_count += 1
                                                                    logger.info(f"[UPLOAD] ✅ Deleted duplicate: {duplicate.name} (kept oldest: {oldest.name})")
                                                                except Exception as e:
                                                                    logger.warning(f"[UPLOAD] Could not delete duplicate {duplicate.name}: {e}")
                                                            
                                                            if deleted_count > 0:
                                                                logger.info(f"[UPLOAD] ✅ Cleaned up {deleted_count} duplicate(s), kept oldest: {oldest.name}")
                                                                print(f"✅ Removed {deleted_count} duplicate file(s), kept oldest")
                                                                # Update filepath_str if we deleted the one we just copied
                                                                if Path(filepath_str) in files_list[1:]:
                                                                    filepath_str = str(oldest)
                                                except Exception as cleanup_err:
                                                    logger.warning(f"[UPLOAD] Duplicate cleanup error: {cleanup_err}")
                                        ...
                                        try:
                                            ...
                                            if not isinstance(alt_paths, list):
                                                alt_paths = []
                                            # Only add original if it's different from workspace path
                                            if filepath_str != str(existing_file_found) if existing_file_found else filepath_str != str(ws_csv_path):
                                                alt_paths.append(filepath_str)
                                            callback_context.state["alt_dataset_paths"] = alt_paths
                                        except (AttributeError, TypeError):
                                            pass
                                        # filepath_str already updated above (either to existing_file_found or ws_csv_path)
                                        logger.info(f"[FIX #20-3] Binding to workspace copy: {Path(filepath_str).name}")
...
# ============================================================================
# PLOT TOOL - Working implementation from old adk_safe_wrappers.py
# ============================================================================
