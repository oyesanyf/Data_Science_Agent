...
def _as_blocks(tool_name: str, result: dict):
    ...
    if isinstance(result, dict):
        ...
        logger.debug(
            f"[_as_blocks] __display__ present: {
                '__display__' in result}")
        if '__display__' in result:
            logger.debug(
                f"[_as_blocks] __display__ value: {str(result.get('__display__', 'MISSING'))[:200]}")
        logger.debug(f"[_as_blocks] message present: {'message' in result}")
        if 'message' in result:
            logger.debug(
                f"[_as_blocks] message value: {str(result.get('message', 'MISSING'))[:200]}")
    ...
    # BUT: Filter out LLM-generated "NEXT STEPS" recommendations from tool
    # output
    display_field = result.get("__display__") or result.get("display") or result.get(
        "message") or result.get("ui_text") or result.get("text")
    if display_field and isinstance(
            display_field, str) and len(
            display_field.strip()) > 0:
        ...
        # BUT: Only filter if it's clearly an LLM addition (not part of
        # legitimate tool output)
        original_length = len(display_field)
        original_display = display_field

        # NEVER filter menu for analyze_dataset_tool - it's part of the tool's
        # legitimate output
        ...
        # CRITICAL: For analyze_dataset_tool with menu, NEVER filter ANYTHING -
        # preserve entire __display__
        if is_analyze_dataset and has_menu:
            logger.info(
                f"[_as_blocks] ✅ Preserving complete menu for analyze_dataset_tool ({
                    len(display_field)} chars)")
            # Keep the entire display field - don't filter anything

        if not is_analyze_dataset and "[NEXT STEPS]" in display_field:
            # Split on NEXT STEPS marker and keep only the tool output part
            # (before NEXT STEPS)
            parts = display_field.split("[NEXT STEPS]")
            if parts and parts[0] and len(
                    parts[0].strip()) > 10:  # Only filter if there's substantial content before it
                display_field = parts[0].strip()
                logger.info(
                    f"[_as_blocks] 🔧 Removed 'NEXT STEPS' section (was {original_length} chars, now {
                        len(display_field)} chars)")
            else:
                # Not enough content before NEXT STEPS - might be legitimate,
                # keep it
        ...
        # BUT: Only filter if Stage appears near the end, not in legitimate
        # content
        if "**Stage" in display_field:
            # Check if Stage appears in the last 20% of the content (likely an
            # addition)
            stage_pos = display_field.find("**Stage")
            if stage_pos > len(display_field) * \
                    0.8:  # Only filter if Stage is in last 20%
                stage_parts = display_field.split("**Stage")
                if stage_parts and stage_parts[0] and len(
                        stage_parts[0].strip()) > 10:
                    display_field = stage_parts[0].strip()
                    logger.info(
                        f"[_as_blocks] 🔧 Removed 'Stage' section (was at position {stage_pos}/{len(original_display)})")

        if len(display_field.strip()) > 0:
            logger.info(
                f"[_as_blocks] ✅ FOUND DISPLAY CONTENT ({
                    len(display_field)} chars), creating markdown block")
            logger.debug(
                f"[_as_blocks] Display content preview: {display_field[:300]}")
            ...
            blocks.append(
                {"type": "markdown", "title": "Summary", "content": display_field})
            logger.info(
                f"[_as_blocks] ✅ Created markdown block with {
                    len(display_field)} chars of content")

        # Also add artifacts if present
        arts = result.get("artifacts") or result.get(
            "artifact_names") or result.get("plots") or []
        if isinstance(arts, list) and arts:
            ...
            if filenames:
                blocks.append({"type": "artifact_list",
                               "title": "Generated Artifacts",
                               "files": filenames})

        logger.info(
            f"[_as_blocks] ✅ Returning {
                len(blocks)} blocks with display content")
        return blocks
    else:
        logger.warning(
            f"[_as_blocks] ⚠️ NO display content found, will try to extract from nested data...")

    # Extract text content (CHECK __display__ FIRST - highest priority!)
    txt = result.get("__display__") or result.get("display") or result.get("ui_text") or result.get(
        "message") or result.get("content") or result.get("summary") or result.get("text") or result.get("_formatted_output")

    logger.info(
        f"[_as_blocks] Extracted txt type: {
            type(txt)}, length: {
            len(
                str(txt)) if txt else 0}, preview: {
                    str(txt)[
                        :150] if txt else None}")

    # CRITICAL: Check if txt is empty or just whitespace (even if _ensure_ui_display set it but it's empty)
    # Also check if txt is a generic message like "completed successfully" -
    # if so, try to extract real data
    ...
    if isinstance(txt, str) and txt.strip():
        ...
        txt_is_generic = any(
            pattern in txt_lower for pattern in generic_patterns) and len(
            txt.strip()) < 100
        # Also check if txt doesn't contain actual data indicators
        has_data_indicators = any(
            indicator in txt_lower for indicator in [
                'shape',
                'rows',
                'columns',
                'mean',
    ...
    # CRITICAL FIX: If no display text found OR it's empty/whitespace OR it's
    # generic, extract from nested 'result' key
    if txt_is_empty or txt_is_generic:
        logger.info(
            f"[_as_blocks] Display text is {
                'empty' if txt_is_empty else 'generic'}, attempting to extract from nested 'result' key...")
        logger.info(f"[_as_blocks] Current txt: {txt[:100] if txt else None}")
        nested_result = result.get("result")
        logger.info(
            f"[_as_blocks] nested_result type: {
                type(nested_result)}, is_dict: {
                isinstance(
                    nested_result,
                    dict)}")
        if isinstance(nested_result, dict):
            logger.info(
                f"[_as_blocks] nested_result keys: {
                    list(
                        nested_result.keys())}")
        if nested_result and isinstance(nested_result, dict):
            # Build detailed message from nested result (same logic as
            # _ensure_ui_display)
            data_parts = []
                    ...
                    if "columns" in overview:
                        ...
                        if isinstance(cols, list):
                            if len(cols) <= 10:
                                ...
                                data_parts.append(
                                    f"**Columns ({len(cols)}):** {', '.join(str(c) for c in cols[:10])}...")
                ...
                if isinstance(num_sum, dict) and num_sum:
                    ...
                    for col, stats in list(num_sum.items())[:5]:
                        if isinstance(stats, dict):
                            ...
                            if isinstance(
                                    mean_val, (int, float)) and isinstance(
                                    std_val, (int, float)):
                                data_parts.append(
                ...
                if isinstance(cat_sum, dict) and cat_sum:
                    ...
                    for col, info in list(cat_sum.items())[:5]:
                        if isinstance(info, dict):
                            ...
                            data_parts.append(
                                f"  • **{col}**: {unique_count} unique values (most common: {top_value})")
                    if len(cat_sum) > 5:
                        data_parts.append(
                            f"  *...and {len(cat_sum) - 5} more*")
                        ...
                        for pair in strong[:3]:
                            if isinstance(pair, dict):
                                ...
                                if isinstance(corr_val, (int, float)):
                                    data_parts.append(
                                        f"  • {col1} ↔ {col2}: {corr_val:.3f}")
                        if len(strong) > 3:
                            data_parts.append(
                                f"  *...and {len(strong) - 3} more*")
            ...
            if "outliers" in nested_result:
                ...
                if isinstance(outliers, dict):
                    outlier_cols = [
                        k for k, v in outliers.items() if isinstance(
                            v, dict) and v.get(
                            "count", 0) > 0]
                    if outlier_cols:
                        data_parts.append(
            ...
            # Extract metrics (can be at nested level or top level of
            # nested_result)
            ...
            # This handles tools that return flat structures like {'accuracy':
            # 0.85, 'precision': 0.82, ...}
            top_level_data_keys = [
                'accuracy',
                'precision',
            ...
            # If no specific data extracted, try to extract ANY meaningful data
            # from nested_result
            if not data_parts:
                ...
                for key in meaningful_keys:
                    if key in nested_result:
                        ...
                        if isinstance(val, str) and len(val.strip()) > 10:
                            data_parts.append(
                                f"**{key.replace('_', ' ').title()}:** {val[:500]}")
                            break
                        elif isinstance(val, (dict, list)) and val:
                            import json
                            try:
                                data_parts.append(
                                    f"**{key.replace('_', ' ').title()}:** {json.dumps(val, indent=2, default=str)[:500]}")
                            except BaseException:
                                data_parts.append(
                                    f"**{key.replace('_', ' ').title()}:** {str(val)[:500]}")
                            break
            ...
            if data_parts:
                ...
                txt = f"{header}\n\n" + "\n".join(data_parts)
                logger.info(
                    f"[_as_blocks] ✅ SUCCESS: Extracted {
                        len(data_parts)} data parts from nested result, total length: {
                        len(txt)} chars")
                ...
                logger.warning(
                    f"[_as_blocks] ⚠️ No data parts extracted from nested_result (data_parts is empty)")
                logger.warning(
                    f"[_as_blocks] nested_result keys available: {
                        list(
                            nested_result.keys()) if isinstance(
                            nested_result,
                            dict) else 'N/A'}")
                # Try to extract ANY data at all, even if it doesn't match our
                # patterns
                if isinstance(nested_result, dict) and nested_result:
                    ...
                    if fallback_parts:
                        ...
                        result["ui_text"] = txt
                        logger.info(
                            f"[_as_blocks] ✅ Used fallback extraction with {
                                len(fallback_parts)} items")

    if txt:
        logger.info(
            f"[_as_blocks] ✅ Found display text, length: {len(str(txt))}, using it for UI")
        ...
        blocks.append(
            {"type": "markdown", "title": "Summary", "content": str(txt)})
    else:
        logger.warning(
            f"[_as_blocks] ❌ NO display text found in result - will use fallback!")
    ...
    if isinstance(rows, list) and rows:
        ...
        try:
            ...
            if rdr:
                blocks.append(
                    {"type": "table", "title": "Data Table", "rows": rdr})
        except Exception:
            pass
    ...
    if not blocks:
        logger.warning(
            f"[_as_blocks] No blocks created for {tool_name}! Creating fallback...")
        ...
        if result:
            ...
            for key in [
                ...
                if key in result and result[key]:
                    fallback_parts.append(
                        f"**{key.title()}:** {str(result[key])[:300]}")
                    break
            ...
            try:
                ...
                if isinstance(nested, dict) and nested:
                    # List nested keys and include a short pretty-printed JSON
                    # preview
                    nested_keys = ", ".join(list(nested.keys())[:10])
                    fallback_parts.append(
                        f"**Nested Result Keys:** {nested_keys}")
                    import json as _json
                    preview = _json.dumps(nested, indent=2, default=str)
                    if len(preview) > 1000:
                        preview = preview[:1000] + "..."
                    fallback_parts.append(
                        f"**Nested Result (preview):**\n\n```json\n{preview}\n```")
                elif isinstance(nested, (list, tuple)) and len(nested) > 0:
                    fallback_parts.append(
                        f"**Nested Result (list length):** {len(nested)}")
                elif isinstance(nested, str) and nested.strip():
                    fallback_parts.append(
                        f"**Nested Result (text):** {nested[:500]}")
            except Exception:
                pass

        fallback_content = "\n\n".join(fallback_parts)
        logger.debug(
            f"[_as_blocks] FALLBACK CONTENT: {fallback_content[:500]}")
        blocks.append({"type": "markdown", "title": "Result",
    ...
    if not blocks or not isinstance(
        ...
        isinstance(
            b,
            dict) for b in blocks):
        logger.warning(
            "[_as_blocks] Invalid or empty UI blocks, adding fallback")
        blocks = [{"type": "markdown", "title": "Result", "content": str(result)[
            :500]}]
    return blocks
...
async def force_publish_to_ui(
    ...
    *,
    ...
    """
    logger.debug(
        f"[force_publish_to_ui] tool_name: {tool_name}, result type: {
            type(result)}")
    if isinstance(result, dict):
        logger.debug(
            f"[force_publish_to_ui] result keys: {
                list(
                    result.keys())}")
        logger.debug(
            f"[force_publish_to_ui] __display__: {
                result.get('__display__')[
                    :200] if result.get('__display__') else None}")
    ...
    try:
        ...
        # Build UI blocks (this also mirrors
        # __display__/message/ui_text/content)
        logger.debug(
            f"[force_publish_to_ui] Calling _as_blocks for {tool_name}")
        blocks = _as_blocks(tool_name, result)
        logger.debug(
            f"[force_publish_to_ui] _as_blocks returned {
                len(blocks)} blocks")
        if not blocks or not isinstance(
            blocks,
            list) or not all(
        ...
        outfile = outdir / \
            f"{tool_name}_exec_{int(datetime.now().timestamp())}.json"
        payload = {
            "tool_name": tool_name,
...
async def after_tool_callback(
    *,
    ...
    """
    logger.debug(
        f"[after_tool_callback] Processing tool: {
            tool.name if tool else 'unknown'}")
    try:
        logger.debug(f"[force_publish_to_ui] RAW result type: {type(result)}")
        if isinstance(result, dict):
            logger.debug(
                f"[force_publish_to_ui] RAW result keys: {
                    list(
                        result.keys())}")
            if '__display__' in result:
                logger.debug(
                    f"[force_publish_to_ui] RAW __display__ preview: {
                        str(
                            result.get(
                                '__display__',
                                ''))[
    ...
    try:
        ...
        tool_name = getattr(
            tool,
            'name',
            str(tool)) if tool is not None else str(
            kwargs.get(
                'tool_name',
                'unknown'))
        callback_context = tool_context or kwargs.get(
            'callback_context') or kwargs.get('tool_ctx')
        if callback_context is None:
            # Nothing to do if no context is provided
            return None

        # Ensure state exists for workflow tracking
        if not hasattr(
                callback_context,
                'state') or callback_context.state is None:
        ...
        if isinstance(result, dict):
            ...
            if 'message' in result:
                logger.info(
                    f"[CALLBACK] Message preview: {str(result['message'])[:200]}")
        ...
        should_replace = result is None or (
            isinstance(
                result,
                dict) and result.get("result") is None and not any(
        ...
        if should_replace:
            logger.warning(
                f"[CALLBACK] Result has no meaningful content! Attempting to load from artifacts...")
            logger.info(
                f"[CALLBACK] Result keys: {
                    list(
                        result.keys()) if isinstance(
                        result,
                        dict) else 'N/A'}")
            logger.info(
                f"[CALLBACK] Has __display__: {
                    '__display__' in result if isinstance(
                        result, dict) else False}")
            logger.info(
                f"[CALLBACK] Has message: {
                    'message' in result if isinstance(
                        result, dict) else False}")
            logger.info(
                f"[CALLBACK] Has ui_text: {
                    'ui_text' in result if isinstance(
                        result, dict) else False}")
            logger.info(
                f"[CALLBACK] Has content: {
                    'content' in result if isinstance(
                        result, dict) else False}")
            logger.info(
                f"[CALLBACK] Has text: {
                    'text' in result if isinstance(
                        result, dict) else False}")
        else:
            logger.info(
                f"[CALLBACK] Result has meaningful content, keeping as-is")
            if isinstance(result, dict):
                logger.info(
                    f"[CALLBACK] Keeping result with keys: {
                        list(
                            result.keys())}")
                if "__display__" in result:
                    logger.info(
                        f"[CALLBACK] __display__ length: {len(str(result['__display__']))}")
            ...
            try:
                ...
                if workspace_root:
                    ...
                    artifact_path = Path(workspace_root) / \
                        "reports" / f"{tool_name}_output.md"
                    if artifact_path.exists():
                        content = artifact_path.read_text(encoding="utf-8")
                        logger.info(
                            f"[CALLBACK] ✓ Loaded {
                                len(content)} chars from {
                                artifact_path.name}")
                        result = {
                            "status": "success",
                            "__display__": content,
                        ...
                        logger.warning(
                            f"[CALLBACK] Artifact not found: {artifact_path}")
            except Exception as e:
        ...
        # If result is still None (ADK may omit on streaming), default to
        # success
        if result is None:
        ...
        try:
            ...
            logger.warning(
                f"[CALLBACK] Artifact routing failed for {tool_name}: {e}")

        # CRITICAL: For analyze_dataset_tool, store the menu in state so it can
        # be displayed
        if "analyze_dataset" in tool_name.lower() and isinstance(result, dict):
            display_content = result.get(
                "__display__") or result.get("message") or ""
            if "SEQUENTIAL WORKFLOW MENU" in display_content or "NEXT STEPS" in display_content:
                ...
                logger.info(
                    f"[CALLBACK] ✅ Stored menu in state for {tool_name} ({
                        len(display_content)} chars)")

        # Store canonical last result for chain steps
        callback_context.state["temp:last_result_kind"] = tool_name
        callback_context.state["temp:last_result_ok"] = result.get(
            "status", "success") == "success"
        callback_context.state["temp:last_result_status"] = result.get(
            "status", "success")

        # Helper to sanitize values for session state (must be
        # deepcopy/picklable)
        def _safe(value):
            try:
        ...
        if tool_name == "describe":
            ...
            if "model" in result:
                callback_context.state["last_model"] = _safe(
                    result["model"])  # avoid storing raw model object
            if "metrics" in result:
                callback_context.state["last_metrics"] = _safe(
                    result["metrics"])
            if "accuracy" in result:
                callback_context.state["last_accuracy"] = _safe(
                    result["accuracy"])

        elif tool_name == "ensemble":
            # Store ensemble results
            if "ensemble_metrics" in result:
                callback_context.state["last_ensemble_metrics"] = _safe(
                    result["ensemble_metrics"])
            if "models_used" in result:
                callback_context.state["last_ensemble_models"] = _safe(
                    result["models_used"])

        elif tool_name in ["impute_simple", "impute_knn", "impute_iterative"]:
            # Store imputation results
            if "imputed_columns" in result:
                callback_context.state["last_imputed_columns"] = _safe(
                    result["imputed_columns"])
            if "imputation_methods" in result:
                callback_context.state["last_imputation_methods"] = _safe(
                    result["imputation_methods"])
        ...
        if result.get("status") == "success":
            ...
            callback_context.state["last_error"] = result.get(
                "error", "Unknown error")
        ...
        try:
            ...
            # Check for errors first - display error message prominently if
            # tool failed
            has_error = False
            error_message = ""
            ...
            if isinstance(result, dict):
                ...
                error_msg_field = result.get(
                    "error_message") or result.get("message", "")

                if tool_status in ("failed", "error") or error_field or (
                        error_msg_field and "error" in error_msg_field.lower()):
                    has_error = True
            ...
            if tool_stage > 0:
                ...
                try:
                    ...
                    stage_name = stage_info.get(
                        'name', f'Stage {tool_stage}') if stage_info else f'Stage {tool_stage}'
                    stage_icon = stage_info.get(
                        'icon', '📋') if stage_info else '📋'
                except BaseException:
                    stage_name = f'Stage {tool_stage}'
                    stage_icon = '📋'

                if has_error:
                    # ERROR CASE: Show error prominently, don't advance
                    # workflow
                    error_banner = (
                        "\n\n" + "=" * 60 + "\n"
                    ...
                    if isinstance(result, dict):
                        existing_display = result.get(
                            "__display__", "") or result.get(
                            "message", "") or ""
                        result["__display__"] = error_banner + existing_display
                        result["message"] = result["__display__"]
                        logger.warning(
                            f"[WORKFLOW] ❌ {tool_name} FAILED - showing error banner, NOT advancing workflow")
                    ...
                    if isinstance(result, dict):
                        # CRITICAL: Check if status has already been shown for
                        # this stage to prevent duplicates
                        status_shown_key = f'stage_{tool_stage}_status_shown'
                        if not callback_context.state.get(status_shown_key):
                            result["__display__"] = existing_display + \
                                status_summary
                            result["message"] = result["__display__"]
                            callback_context.state[status_shown_key] = True
                            logger.debug(
                                f"[WORKFLOW] Added error status for stage {tool_stage}")
                        else:
                            logger.debug(
                                f"[WORKFLOW] Status already shown for stage {tool_stage}, skipping duplicate")

                elif result.get("status") == "success" or result.get("status") == "warning":
                    # SUCCESS/WARNING CASE: Add status summary and advance to
                    # next stage
                    status_icon = "✅" if result.get(
                        "status") == "success" else "⚠️"
                    status_word = "COMPLETED" if result.get(
                        "status") == "success" else "COMPLETED (WITH WARNINGS)"
                    status_summary = (
                        "\n\n" + "-" * 60 + "\n"
                    ...
                    if next_stage_id > current_stage and next_stage_id <= 14:
                        ...
                        from .workflow_stages import get_stage
                        stage_info = get_stage(tool_stage)
                        user_must_choose = stage_info.get('user_must_choose', False)

                        if user_must_choose:
                            logger.info(f"[WORKFLOW] Stage {tool_stage} requires user to choose. Not showing next stage menu.")
                            if isinstance(result, dict):
                                status_shown_key = f'stage_{tool_stage}_status_shown'
                                if not callback_context.state.get(
                                        status_shown_key):
                                    existing_display = result.get(
                                        "__display__", "") or result.get(
                                        "message", "") or ""
                            ...
                            if isinstance(result, dict):
                                # CRITICAL: Check if status has already been shown
                                # for this stage to prevent duplicates
                                status_shown_key = f'stage_{tool_stage}_status_shown'
                                if not callback_context.state.get(
                                        status_shown_key):
                                    existing_display = result.get(
                                    ...
                                    callback_context.state[status_shown_key] = True
                                    logger.info(
                                        f"[WORKFLOW] ✅ Stage {tool_stage} ({stage_name}) COMPLETED → Advanced to Stage {next_stage_id}: {
                                            next_stage['name']}")
                                else:
                                    logger.debug(
                                        f"[WORKFLOW] Status already shown for stage {tool_stage}, skipping duplicate")
                                ...
                                logger.debug(
                                    f"[WORKFLOW] Result is not dict, cannot append menu")
                    else:
                        ...
                        if isinstance(result, dict):
                            ...
                            if not callback_context.state.get(
                                ...
                                callback_context.state[status_shown_key] = True
                                logger.debug(
                                    f"[WORKFLOW] Added status for stage {tool_stage}")
                            else:
                                logger.debug(
                                    f"[WORKFLOW] Status already shown for stage {tool_stage}, skipping duplicate")
                        logger.debug(
                            f"[WORKFLOW] Stage {next_stage_id} already shown or complete")
                else:
                    ...
                    if isinstance(result, dict):
                        ...
                        if not callback_context.state.get(status_shown_key):
                            ...
                            callback_context.state[status_shown_key] = True
                            logger.debug(
                                f"[WORKFLOW] Added unknown status for stage {tool_stage}")
                        else:
                            logger.debug(
                                f"[WORKFLOW] Status already shown for stage {tool_stage}, skipping duplicate")
                    logger.debug(
                        f"[WORKFLOW] Tool {tool_name} status is '{
                            result.get('status')}' (not success/error)")
            else:
                logger.debug(
                    f"[WORKFLOW] Tool {tool_name} not recognized for stage advancement (stage={tool_stage})")
        except Exception as e:
            logger.error(
                f"[WORKFLOW] Error advancing workflow stage: {e}",
                exc_info=True)
        ...
        try:
            ...
            if s.get("default_csv_path") and not s.get(
                    "mirrored_upload_saved"):
                from google.genai import types as gen_types
                payload = f"file_id={
                    s.get('default_corpus_id') or s.get(
                        'last_file_id', '')}\npath={
                    s['default_csv_path']}\n".encode("utf-8")
                part = gen_types.Part.from_bytes(
        ...
        if result and isinstance(result, dict):
            # CRITICAL: Check if artifacts list has items, not just if list
            # exists
            ...
            has_artifacts = (
                isinstance(
                    artifacts_list,
                    list) and len(artifacts_list) > 0) or (
                isinstance(
                    plots_list,
                    list) and len(plots_list) > 0)
            if (result.get("status") == "success") and not has_artifacts and tool_name.lower(
            ).startswith("plot"):
                result["status"] = "failed"
                result["message"] = "Plot tool reported success but produced no artifacts."
                logger.warning(
                    f"[CALLBACK] Plot tool returned success but no artifacts found (artifacts={
                        len(artifacts_list) if isinstance(
                            artifacts_list, list) else 'invalid'}, plots={
                        len(plots_list) if isinstance(
                            plots_list, list) else 'invalid'})")

        # [OK] Promote tool result text to the chat stream as an assistant message
        try:
            if isinstance(result, dict):
                ui_text = result.get("__display__") or result.get("ui_text") or result.get("message") or result.get(
                    "summary") or result.get("text") or result.get("display") or result.get("content")
                if ui_text and ui_text.strip():
                    # Log that we're trying to display this
                    logger.info(
                        f"[CALLBACK] Promoting tool result to UI: {ui_text[:100]}...")
                    ...
                    try:
                        ...
                        # Note: This depends on ADK version and runner
                        # implementation
                        if hasattr(callback_context, 'add_assistant_message'):
                            callback_context.add_assistant_message(text_part)
                            logger.info(
                                "[CALLBACK] [OK] Added assistant message via add_assistant_message")
                        elif hasattr(callback_context, 'reply'):
                            callback_context.reply(ui_text[:4000])
                            logger.info(
                                "[CALLBACK] [OK] Added assistant message via reply")
                        elif hasattr(callback_context, 'add_content'):
                            callback_context.add_content(text_part)
                            logger.info(
                                "[CALLBACK] [OK] Added assistant message via add_content")
                        else:
                            # Fallback: store in state for agent to potentially
                            # use
                            callback_context.state["_tool_result_to_display"] = ui_text[:4000]
                            logger.warning(
                                "[CALLBACK] [WARNING] No known method to add assistant message, stored in state")
                    except Exception as e:
                        logger.warning(
                            f"[CALLBACK] Failed to add assistant message: {e}")
                        # Last resort: try setattr
                        setattr(callback_context, "reply_text", ui_text[:4000])
        except Exception as e:
            logger.error(
                f"[CALLBACK] Error promoting tool result: {e}",
                exc_info=True)
        ...
        try:
            if isinstance(result, dict):
                ...
                seen = set()
                unique_paths = [
                    p for p in paths if p and p not in seen and not seen.add(p)]
                ...
                for p in unique_paths:
                    try:
                        ...
                        mime = mimetypes.guess_type(
                            p)[0] or "application/octet-stream"
                        ...
                        logger.info(
                            f"[CALLBACK] [OK] Pushed artifact to UI: {name}")
                    except Exception as e:
                        logger.warning(
                            f"[CALLBACK] Failed to push artifact {p}: {e}")
                ...
                if saved_names:
                    ...
                    existing = result.get("__display__") or result.get(
                        "ui_text") or result.get("message") or ""
                    updated_text = (
            ...
            logger.error(
                f"[CALLBACK] Error in artifact routing: {e}",
                exc_info=True)
        ...
        try:
            ...
            # This ensures _as_blocks can modify display fields without
            # affecting downstream processing
            import copy
            result_for_ui = copy.deepcopy(
                result) if isinstance(result, dict) else {}
            if not isinstance(result_for_ui, dict) or not result_for_ui:
                # Ensure _as_blocks always has something usable
                result_for_ui = {"__display__": str(
                    result) if result is not None else "No result available"}

            # Log what _as_blocks will receive
            logger.info(
                f"[CALLBACK] Before _as_blocks - result_for_ui keys: {
                    list(
                        result_for_ui.keys()) if isinstance(
                        result_for_ui,
                        dict) else 'N/A'}")
            if isinstance(result_for_ui, dict):
                logger.info(
                    f"[CALLBACK] __display__ in result_for_ui: {
                        '__display__' in result_for_ui}")
                if '__display__' in result_for_ui:
                    logger.info(
                        f"[CALLBACK] __display__ value preview: {str(result_for_ui['__display__'])[:200]}")

            # Convert result to UI blocks (may modify result_for_ui to add
            # __display__)
            blocks = _as_blocks(tool_name, result_for_ui)

            # Log what _as_blocks returned
            logger.info(
                f"[CALLBACK] After _as_blocks - blocks count: {
                    len(blocks)}, block types: {
                    [
                        b.get('type') for b in blocks]}")

            # CRITICAL: Merge any display fields that _as_blocks extracted back into main result
            # This ensures the extracted data is preserved for clean_for_json
            # and return
            if isinstance(
                    result_for_ui,
                    dict) and result_for_ui.get("__display__"):
                result["__display__"] = result_for_ui["__display__"]
                result["message"] = result_for_ui.get(
                    "message") or result_for_ui["__display__"]
                result["ui_text"] = result_for_ui.get(
                    "ui_text") or result_for_ui["__display__"]
                result["content"] = result_for_ui.get(
                    "content") or result_for_ui["__display__"]
                logger.info(
                    f"[CALLBACK] ✓ Merged extracted display from _as_blocks back into result (length: {len(str(result['__display__']))})")

            # Publish to UI page (creates/updates session_ui_page.md in
            # Artifacts)
            logger.info(
                f"[CALLBACK] About to call publish_ui_blocks for tool: {tool_name}")
            await publish_ui_blocks(callback_context, tool_name, blocks)
            logger.info(
                f"[CALLBACK] Completed publish_ui_blocks for tool: {tool_name}")
            ...
            success = result.get(
                "status", "success") == "success" if isinstance(
            ...
            logger.debug(
                f"[UI SINK] Published {
                    len(blocks)} blocks for {tool_name}")
        except Exception as e:
            ...
            if isinstance(result, dict) and result.get("__display__"):
                ...
                if not workspace_root:
                    try:
                        ...
                        artifact_manager.ensure_workspace(
                            callback_context.state, UPLOAD_ROOT)
                        workspace_root = callback_context.state.get(
                            "workspace_root")
                        if workspace_root:
                            logger.info(
                                f"[REPORT DATA] ✓ Created workspace for output saving: {workspace_root}")
                    except Exception as e:
                        logger.warning(
                            f"[REPORT DATA] Could not create workspace: {e}")
                ...
                if workspace_root:
                    ...
                    # Create results directory if it doesn't exist (NEW:
                    # dedicated results/ folder for structured outputs)
                    results_dir = Path(workspace_root) / "results"
                    results_dir.mkdir(parents=True, exist_ok=True)

                    # Save output with timestamp
                    output_file = results_dir / \
                        f"{tool_name}_output_{int(datetime.now().timestamp())}.json"
                    ...
                    logger.info(
                        f"[REPORT DATA] ✓ Saved {tool_name} output: {
                            output_file.name}")
                else:
                    logger.warning(
                        f"[REPORT DATA] Cannot save {tool_name} output - workspace_root not available")
        except Exception as e:
            logger.error(
                f"[REPORT DATA] Failed to save tool output for report: {e}",
                exc_info=True)

        # ===== CRITICAL: Normalize result for UI display =====
        # Implement recommended sanitization functions for bullet-proof
        # serialization
        import math
        ...
        def clean_for_json(obj, depth=0):
            ...
            # CRITICAL: Check for async generators, generators, coroutines
            # FIRST
            if inspect.isasyncgen(obj) or inspect.isgenerator(
            ...
            # because numpy.float64 is isinstance(float) but still needs
            # conversion!
            ...
            if obj_type == 'numpy' or obj_type.startswith('numpy.'):
                ...
                if hasattr(obj, 'tolist'):
                    try:
                        result = obj.tolist()
                        # Recursively clean the result (might contain more
                        # numpy types)
        ...
        try:
            ...
            if inspect.isasyncgen(result) or inspect.isgenerator(
                    result) or inspect.iscoroutine(result):
                logger.warning(
                    f"[CALLBACK] Detected async_generator/generator/coroutine from {tool_name} - not supported")
                return {
                    "status": "error",
                    "message": f"{tool_name} returned unsupported async generator type. Use non-streaming version."}
            ...
            def has_async_generator(obj, depth=0):
                ...
                if inspect.isasyncgen(obj) or inspect.isgenerator(
                    ...
                    return True
                if isinstance(obj, dict):
                    return any(has_async_generator(v, depth + 1)
                               for v in obj.values())
                if isinstance(obj, (list, tuple)):
                    return any(has_async_generator(item, depth + 1)
                               for item in obj)
                return False

            if has_async_generator(result):
                logger.warning(
                    f"[CALLBACK] Found async_generator inside {tool_name} result - not supported")
                return {
                    "status": "error",
                    "message": f"{tool_name} returned unsupported async generator type. Use non-streaming version."}
                ...
                if not display_text:
                    ...
                    if status == "success":
                        ...
                        artifacts = result.get(
                            "artifacts") or result.get("artifact") or []
                        if artifacts:
            ...
            # Returning None would let the ORIGINAL (uncleaned) result flow
            # through with async generators!

            # Check if result has the minimum required display fields
            has_display = any(
                result.get(f) for f in [
                    "__display__",
                    "message",
                    "text",
                    "ui_text"])
            ...
            if not has_display:
                ...
                logger.warning(
                    f"[CALLBACK] Result missing display fields, adding default")
                result["__display__"] = f"✅ **{tool_name}** completed"
                result["message"] = result.get(
                    "message") or f"{tool_name} completed successfully"

            logger.info(
                f"[CALLBACK] Returning cleaned result (has display: {has_display})")
            # CRITICAL: Return the cleaned result (with async generators removed)
            # Do NOT return None - that would use the original uncleaned
            # result!
            return result

        except Exception as e:
            logger.error(
                f"[CALLBACK] Error in after_tool_callback: {e}",
                exc_info=True)
            # Return safe fallback - don't let potentially broken result
            # through
            return {
                "status": "success",
...
def after_upload_callback_DISABLED(
    *,
    tool_context=None,
    result=None,
        **kwargs):
    ...
    if result is None:
        logger.info(
            "!!!!!!!!!!!!!!!!! after_upload_callback NO RESULT !!!!!!!!!!!!!!!!!")
        return
    ...
    if file_path and tool_context:
        ...
        logger.info(
            f"!!!!!!!!!!!!!!!!! Set default_csv_path to {file_path} !!!!!!!!!!!!!!!!!")
    ...
    try:
        ...
        logger.info(
            "!!!!!!!!!!!!!!!!! Triggered _bootstrap_step1_and_menu !!!!!!!!!!!!!!!!!")
    except Exception as e:
        logger.debug(
            f"[after_upload_callback] Failed to trigger _bootstrap_step1_and_menu: {e}")
